...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build query parameters for top gainers (highest 24h price increase)
    const params = new URLSearchParams({
      vs_currency: vsCurrency,
      order: "price_change_percentage_24h_desc", // Descending order for top gainers
      per_page: perPage,
      page: page,
      sparkline: sparkline,
    ...
    // Set caching headers (2 minutes cache for top gainers)
    const headers = {
