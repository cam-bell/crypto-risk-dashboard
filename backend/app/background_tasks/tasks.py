...
def fetch_crypto_prices():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Successfully fetched prices for {len(result)} cryptocurrencies"
            )
            return result
        finally:
            loop.close()
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                _fetch_wallet_analysis_async(wallet_addresses)
            )
...
async def _fetch_wallet_analysis_async(
    ...
    async with EtherscanClient() as client:
        for address in wallet_addresses:
            try:
                ...
                # Get token transfers
                token_response = await client.get_erc20_token_transfers(
                    address, offset=100
                )
                ...
                results.append({"address": address, "data": wallet_data})
...
async def _health_check_apis_async() -> Dict[str, Any]:
    ...
    await cache.cache_api_response("system", "health_check", {}, results, ttl=300)
...
def fetch_specific_crypto_data(crypto_id: str, data_type: str = "price"):
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                _fetch_specific_crypto_data_async(crypto_id, data_type)
            )
...
async def _fetch_specific_crypto_data_async(
    crypto_id: str, data_type: str
) -> Dict[str, Any]:
