...
class AIInsight(Base):
    ...
    portfolio_id = Column(
        String, ForeignKey("portfolios.id"), nullable=True, index=True
    )
    crypto_asset_id = Column(
        String, ForeignKey("crypto_assets.id"), nullable=True, index=True
    )

    # Insight details
    insight_type = Column(
        String(50), nullable=False, index=True
    )  # risk, opportunity, trend, etc.
    ...
    def __repr__(self):
        return (
            f"<AIInsight(id={self.id}, type={self.insight_type}, title={self.title})>"
        )
