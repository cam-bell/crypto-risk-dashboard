...
class BaseAPIClient(ABC):
    ...
    async def _make_request(
        ...
        try:
            ...
            return await self._handle_retry(method, endpoint, params, headers, data, retry_count, "timeout")
            
        except HTTPStatusError as e:
            logger.warning(f"HTTP error {e.response.status_code} for {url}: {e}")
            return await self._handle_retry(method, endpoint, params, headers, data, retry_count, "http_error")
            
        except Exception as e:
            logger.error(f"Unexpected error for {url}: {e}")
            return await self._handle_retry(method, endpoint, params, headers, data, retry_count, "unexpected_error")
    ...
    async def _handle_retry(
        ...
        if retry_count < self.max_retries:
            ...
            logger.info(f"Retrying {method} {endpoint} in {wait_time}s (attempt {retry_count}/{self.max_retries})")
            await asyncio.sleep(wait_time)
            
            return await self._make_request(method, endpoint, params, headers, data, retry_count)
        else:
            return APIResponse(
                success=False,
