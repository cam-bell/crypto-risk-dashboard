...
class CryptoDataIntegrationService:
        ...
        try:
            ...
            if wallet_data:
                ...
                await cache.cache_api_response(
                    "wallet_analysis", "comprehensive", 
                    {"address": wallet_address}, wallet_data
                )
    ...
    async def _get_alphavantage_data(self, crypto_ids: List[str]) -> Optional[Dict[str, Any]]:
    ...
    async def _get_wallet_data(self, wallet_address: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Get balance
            balance_response = await self.etherscan_client.get_account_balance(wallet_address)
            if balance_response.success:
                wallet_data["balance"] = balance_response.data
            
            # Get transactions
            tx_response = await self.etherscan_client.get_account_transactions(wallet_address, offset=50)
            if tx_response.success:
                wallet_data["transactions"] = tx_response.data
            
            # Get token transfers
            token_response = await self.etherscan_client.get_erc20_token_transfers(wallet_address, offset=50)
            if token_response.success:
                wallet_data["token_transfers"] = token_response.data
            
            # Get NFT transfers
            nft_response = await self.etherscan_client.get_nft_transfers(wallet_address, offset=50)
    ...
    async def _get_sentiment_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            sentiment_response = await self.alphavantage_client.get_market_sentiment(symbol)
